        ...
        <div class="row">
            <div class="col-6">
                <div class="media">
                    <div class="media-left">
                        @if ($logoUrl)
                            <img class="media-object" src="{{ $logoUrl }}" alt="{{ $companyName }}"
            ...
            <div class="col-3">
                <div class="text-md-right">
                    ...
                    @endif
                    <p>
                        Ordered At: {{ $order->created_at->format('M') }}<span class="digits">
                            {{ $order->created_at->format('d, Y') }}</span>
                    </p>
                </div>
                <!-- End Title-->
            </div>
            <div class="col-3">
                <div class="text-md-right" id="project">
                    <img height="80"
                        src="https://barcode.tec-it.com/barcode.ashx?data={{ $order->barcode }}&code=Code128&translate-esc=true"
                        alt="Barcode">
                </div>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    @foreach ($order->products as $product)
                        <tr>
                            ...
                            <td>{{ $product->name }}</td>
                            <td>{{ isOninda() && config('app.resell') ? $product->retail_price ?? $product->price : $product->price }}
                            </td>
                            <td>{{ $product->quantity }}</td>
                            <td>{{ $amount = $product->quantity * (float) (isOninda() && config('app.resell') ? $product->retail_price ?? $product->price : $product->price) }}
                            </td>
                        </tr>
                        @php($retail += $amount)
                    @endforeach
                    ...
                    <tr>
                        ...
                        <th class="py-1">
                            {{ isOninda() && config('app.resell') ? $order->data['retail_delivery_fee'] ?? $order->data['shipping_cost'] : $order->data['shipping_cost'] }}
                        </th>
                    </tr>
                    <tr>
