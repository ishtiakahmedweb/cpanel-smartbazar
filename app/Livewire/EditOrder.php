...
class EditOrder extends Component
{
    ...
    private array $meta = [
        ...
        'subtotal', 'courier', 'city_id', 'area_id', 'city_name', 'area_name', 'weight', 'packaging_charge',
        // 'is_fraud', 'is_repeat',
    ];
    ...
    {
        if (! config('services.courier_report.key')) {
        ...
        // Handle specific API error for better UX
        if (isset($report['message']) && str_contains($report['message'], 'subscription')) {
            return 'API Error: Your BD Courier subscription is not active or has expired. Please check your dashboard.';
        }
    ...
    public function mount(Order $order): void
    {
        ...
        $this->city_name = $this->order->data['city_name'] ?? '';
        $this->area_name = $this->order->data['area_name'] ?? '';

        foreach (json_decode(json_encode($this->order->products), true) ?? [] as $product) {
            // Fallback for missing image
            if (empty($product['image'])) {
                $dbProduct = cacheMemo()->remember(
                    'product_base_image:'.$product['id'],
                    now()->addMinutes(5),
                    fn () => Product::find($product['id'])
                );
                if ($dbProduct && $dbProduct->base_image) {
                    $product['image'] = $dbProduct->base_image->src;
                }
            }
    ...
    {
        $city = \App\Models\District::find($value);
        $this->city_name = $city?->name;
    }

    public function updatedAreaId($value): void
    {
        if ($this->courier == 'Other' || empty($this->courier)) {
