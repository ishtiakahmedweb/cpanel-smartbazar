...
class OrderController extends Controller
{
    ...
    public function filter(Request $request)
    {
        ...
        if ($driver === 'sqlite') {
            $totalSQL = 'COUNT(*) as order_count, SUM(json_extract(data, "$.subtotal")) + SUM(json_extract(data, "$.shipping_cost")) - COALESCE(SUM(json_extract(data, "$.discount")), 0) as total_amount';
        } else {
