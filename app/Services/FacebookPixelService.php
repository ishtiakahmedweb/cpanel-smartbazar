...
class FacebookPixelService
{
    ...
    protected function createServerCustomData(array $customData): \FacebookAds\Object\ServerSide\CustomData
    {
        ...
        if (isset($customData['content_ids'])) {
            ...
            foreach ($customData['content_ids'] as $id) {
                $content = new Content;
                $content->setProductId((int) $id);
                $content->setTitle($customData['content_name']);
                $content->setQuantity((int) ($customData['quantity'] ?? 1));
    ...
    protected function createServerUserData(array $userData)
    {
        ...
        if (isset($userData['city'])) {
            $userDataObj->setCity($userData['city']);
        }
        if (isset($userData['country'])) {
            $userDataObj->setCountryCode($userData['country']);
        }
        if (isset($userData['address'])) {
            // FB Pixel/CAPI doesn't have a direct "setAddress" for basic matching, 
            // but we can pass it through custom data if needed. 
            // However, we ensure standard fields are set.
        }
