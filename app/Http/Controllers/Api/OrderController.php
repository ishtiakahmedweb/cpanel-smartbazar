...
class OrderController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        if ($request->type && strtolower($request->type) === 'online') {
            $orders->where('orders.type', Order::ONLINE);
        } elseif ($request->type && strtolower($request->type) === 'manual') {
        ...
        $dt = DataTables::of($orders)
            ...
            ->setRowAttr([
                'style' => function ($row) {
                    if ($row->data['is_fraud'] ?? false) {
                        return 'background-color: #ffe5e5; border-left: 4px solid #cc0000;';
                    }
                    if (! ($row->data['is_fraud'] ?? false) && ($row->data['is_repeat'] ?? false)) {
                        return 'background-color: #fff9e6; border-left: 4px solid #ffcc00;';
                    }
                },
            ])
            ...
            ->editColumn('customer', function ($row) {
                $cityArea = '';
                if ($row->data['city_name'] ?? false) {
                    $cityArea .= "<div><i class='mr-1 fa fa-map-marker text-primary'></i>{$row->data['city_name']}";
                    if ($row->data['area_name'] ?? false) {
                        $cityArea .= " - {$row->data['area_name']}";
                    }
                    $cityArea .= "</div>";
                }
                ...
                return "
                    <div class='customer-info'>
                        <div style='white-space:nowrap;' class='font-weight-bold'><i class='mr-1 fa fa-user'></i>{$row->name}</div>
                        <div style='white-space:nowrap;'><i class='mr-1 fa fa-phone'></i><a href='tel:{$row->phone}'>".without88($row->phone)."</a></div>
                        {$cityArea}
                        <div style='font-size: 0.85rem; color: #666;'><i class='mr-1 fa fa-home'></i>{$row->address}</div>".
                ($row->note ? "<div class='text-danger mt-1' style='font-size: 0.85rem;'><i class='mr-1 fa fa-sticky-note-o'></i>{$row->note}</div>" : '').
                "</div>";
            })
            ->editColumn('products', function ($row) {
                ...
                foreach ((array) ($row->products) ?? [] as $product) {
                    ...
                    $productLink = isset($product->slug) ? "<a class='text-underline' href='".route('products.show', $product->slug)."' target='_blank'>{$product->name}</a>" : $product->name;
                    $products .= "<li style='margin-bottom: 12px;'><div style='display: flex; align-items: center; margin-bottom: 4px;'>{$product->quantity} x {$imageHtml}</div><div>$productLink</div></li>";
                }
            ...
            ->addColumn('courier', function ($row) {
                ...
                // Show City, Area, and Weight for all orders if available
                if ($row->data['city_name'] ?? false) {
                    $return .= '<div style="white-space: nowrap; font-size: 0.8rem;"><strong>City:</strong> '.$row->data['city_name'].'</div>';
                }
                if ($row->data['area_name'] ?? false) {
                    $return .= '<div style="white-space: nowrap; font-size: 0.8rem;"><strong>Area:</strong> '.$row->data['area_name'].'</div>';
                }
                $return .= '<div style="white-space: nowrap; font-size: 0.8rem;"><strong>Weight:</strong> '.($row->data['weight'] ?? '0.5').' kg</div>';
